...
- Track Title truncation cut text well before the rect edge because `_trackTitleMaxChars` was a char count tuned for all-Japanese worst-case width (632px / fontSize 28 ≈ 22 full-width chars). Mixed/English content left a lot of empty space. Switch the Title to a `VerticalLayoutGroup` + `ContentSizeFitter` + `LayoutElement` setup with TMP wrap=true so titles wrap to 2 (or rarely 3) lines and each track row's height auto-fits the wrapped content. Keep `_trackTitleMaxChars` (renamed in spirit to "max weight") as a 200-weight safety cap for runaway input. `RenderTrackList` no longer manually positions clones or sets parent sizeDelta — VLG and CSF do it (#23 polish, user-reported)
