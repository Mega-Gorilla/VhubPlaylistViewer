...
│   ├── Scroll View            (anchoredPos y=-42, size 720×636) — 結果カードリスト (#38 で footer SearchBar 挿入のため縮小、旧 720×780)
...
6. **SearchView `Scroll View`** repos: anchoredPos (0,**-42**) size (720,**636**) で `#TabRow` 下端 (canvas y=284) の 8px 下から `#SearchBar` 上端 (canvas y=-376) の 16px 上まで配置 (#38 で footer SearchBar 挿入のため旧 720×780 から縮小、track 同時表示数 ~10→~8)。算式: `top_y=276` (=TabRow bottom 284 -8 padding) / `bottom_y=-360` (=SearchBar top -376 +16 padding) / `height=636` / `anchoredPos.y=(276+(-360))/2=-42` / `sizeDelta=(720,636)`
...
10. **DetailView `Scroll View`** resize: anchoredPos (0,-101.5) sizeDelta (-40,-645) (高さ 379、top y=+88 維持、bottom y=-291、`#UrlField` 上端 (canvas y=-307) の +16px 上)。**polish PR で縦拡張** (旧値: anchoredPos (0,-85.5) sizeDelta (-40,-677)、高さ 347): bottom を `#UrlField` 上端の +16px 上まで延長 (track 同時表示数 ~5→~6)。**testing-chamber 実測**: `#UrlField` は anchor (0,0)-(1,0) pivot (0.5,0) anchoredPos (0,155) sizeDelta (-40,50) → top canvas y=-307。scene-specific な UrlField 位置に応じて以下の formula で逆算可 (canvas h=1024 / Scroll View top y=+88 維持 前提):<br>　・`bottom_y = UrlField_top_canvas_y + 16` (gap 16px、bottom は UrlField top より 16px 上 = canvas-y で +16)<br>　・`height = top_y - bottom_y = 88 - (UrlField_top_canvas_y + 16)`<br>　・`sizeDelta.y = height - canvas_height = (88 - (UrlField_top_canvas_y + 16)) - 1024`<br>　・`anchoredPos.y = (top_y + bottom_y) / 2 = (88 + (UrlField_top_canvas_y + 16)) / 2`<br>　検算: UrlField_top=-307 → sizeDelta.y = `(88-(-291))-1024 = 379-1024 = -645` ✓、anchoredPos.y = `(88+(-291))/2 = -101.5` ✓。Image color は polish PR 以降 **surface tint α=0.08** とし、詳細は step 17 参照 (旧 PR #34 では α=0 透過だったが polish PR で revert)
